...
exports.commands = [
    ...
    {
        command: (new SlashCommandBuilder())
            .setName("game_report_nick")
            .setDescription("report a rating game that took place and automatically update players ELO")
            .addStringOption((option) =>
                option.setName('player_a').setDescription('First Player Nick').setRequired(true)
            )
            .addStringOption((option) =>
                option.setName('player_b').setDescription('Second Player Nick').setRequired(true)
            )
            .addNumberOption((option) =>
                option.setName('score_a').setDescription('First Player Score').setMinValue(0).setRequired(true)
            )
        ...
        func: async (interaction)=>{
            const playerAId = await db.getPlayerIdByNick(interaction.options.getString('player_a'));
            const playerBId = await db.getPlayerIdByNick(interaction.options.getString('player_b'));

            const scoreA = parseInt(interaction.options.getNumber('score_a'));
            const scoreB = parseInt(interaction.options.getNumber('score_b'));

            if(!(await db.playerExists(playerAId)) || !(await db.playerExists(playerBId))){
                interaction.reply("One or both players don't exist, use `/set_elo` to add them");
                return;
            }

            let playerADataPregame = await db.getPlayerData(playerAId);
            let playerBDataPregame = await db.getPlayerData(playerBId);

            let [diffA, diffB] = await db.reportGame(playerAId, playerBId, scoreA, scoreB);
